...
const Supply = () => {
  ...
  const fetchSupply = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from("eq_supply_1").select("*");
      if (error) throw error;

      const supplyData = data.map((row) => ({
        id: row.id_eq_supply_1,
        nama: row.nama,
      }));
      setAllData(supplyData);
    } catch (error) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq("id_eq_supply_1", selectedItem.id);
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <h2 className="text-xl font-semibold text-gray-900">Supply</h2>
        ...
        <div className="p-6">
          ...
          <div className="text-center mb-6">
            <p className="text-gray-700">
              Are you sure you want to delete the supply{" "}
